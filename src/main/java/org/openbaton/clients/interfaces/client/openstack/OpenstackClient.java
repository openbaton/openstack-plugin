...
public class OpenstackClient extends VimDriver {
  ...
  public static void main(String[] args)
    ...
    if (args.length == 6)
      ...
      PluginStarter.registerPlugin(
          ...
          Integer.parseInt(args[2]),
          Integer.parseInt(args[3]));
    else PluginStarter.registerPlugin(OpenstackClient.class, "openstack", "localhost", 5672, 10);
  }
  ...
  private String getZone(VimInstance vimInstance) {
    ...
    if (vimInstance.getLocation() != null) {
      if (vimInstance.getLocation().getName() != null) {
        ...
        log.warn(
            "Location of openstack environment is not defined properly -> Missing Name of the Location. Selecting a random one...");
      }
    } else {
  ...
  public Server launchInstanceAndWait(
    ...
    return launchInstanceAndWait(
        vimInstance, name, imageId, flavorId, keypair, network, secGroup, userData, null);
  }
  ...
  private String getNetworkIdByName(VimInstance vimInstance, String key) throws VimDriverException {
    for (Network n : this.listNetworks(vimInstance)) {
      if (n.getName().equals(key)) return n.getExtId();
    }
    return null;
  }
    ...
    try {
      ...
      for (org.jclouds.openstack.nova.v2_0.domain.Server jcloudsServer :
        ...
        if (jcloudsServer.getTenantId().equals(tenantId)) {
          ...
          if (image != null) server.setImage(getImageById(vimInstance, image.getId()));
          else log.warn("The image this server is using was deleted");
          Resource flavor = jcloudsServer.getFlavor();
          if (flavor != null) server.setFlavor(getFlavorById(vimInstance, flavor.getId()));
          else log.warn("The flavor this server is using was deleted");
  ...
  public synchronized void associateFloatingIpToNetwork(
    ...
    try {
      ...
      //throw new VimDriverException(e.getMessage());
      log.warn(
          "It seems that floatingApi is not present or there are not enough available floating ips, this means that we will not be able to assign them");
    } finally {
      if (connection != null) {
        connection.disconnect();
      }
  ...
  public void get_allocated(VimInstance vimInstance, String pool_name, int ipsNeeded) {
    ...
    try {
      ...
      log.warn(
          "It was impossible to allocate more floating ips because the quota is riched or floatingapis are not available");
      //throw new VimDriverException(e.getMessage());
    }
    return;
  }
